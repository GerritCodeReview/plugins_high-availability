...
:   When autoReindex is enabled, indicates the interval between the conditional
    reindexing of all changes and accounts groups.
    Delay is expressed in Gerrit time values as in [websession.cleanupInterval](#websessioncleanupInterval).
...
**Note:** For projects and groups reindexing, [scheduled indexer](https://gerrit-review.googlesource.com/Documentation/config-gerrit.html#scheduledIndexer) must be enabled with specific configurations.
...
[scheduledIndexer "projects"]
    ...
    startTime = 13:00
```
Ensure these settings are added to enable periodic reindexing of groups and projects.
