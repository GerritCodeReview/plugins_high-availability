...
:   When autoReindex is enabled, projectsReindex enables auto reindexing for all projects
    after startup. High-availability plugin cannot determine which project has been changed while
    a node was down. When not specified, the default is false.
