...
When the [`autoReindex.enabled` option](config.md) is set to `true`, this is used
to determine which changes to reindex when a node is temporarily out of sync with the primary,
