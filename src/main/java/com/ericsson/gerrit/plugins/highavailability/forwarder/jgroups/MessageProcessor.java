...
@Singleton
class MessageProcessor implements RequestHandler {
  ...
  public Object handle(Message msg) {
    ...
    try {
      if (cmd instanceof IndexChange) {
        return indexChange(cmd);
      }
      if (cmd instanceof IndexAccount) {
        return indexAccount(cmd);
      }
      if (cmd instanceof IndexGroup) {
        return indexGroup(cmd);
      }
      if (cmd instanceof EvictCache) {
        return evictCache(cmd);
      }
      if (cmd instanceof PostEvent) {
        return forwardEvent(cmd);
      }
    } catch (Exception e) {
      log.error("Unable to handle message {}", msg, e);
    }
    return false;
  }

  private static Command getCommand(Message msg) {
    try {
      String s = (String) msg.getObject();
      log.debug("Received message: {} from {}", s, msg.getSrc());
  ...
  private boolean indexChange(Command cmd) {
    IndexChange indexChange = (IndexChange) cmd;
    Operation op = indexChange.getOperation();
    try {
      forwardedIndexChangeHandler.index(new Change.Id(indexChange.getId()), op);
      log.debug("Change index '{}' on change {} done", op, indexChange.getId());
      return true;
    } catch (Exception e) {
      log.error(
          "Change index '{}' on change {} failed", op.name().toLowerCase(), indexChange.getId(), e);
      return false;
    }
  }

  private Object indexAccount(Command cmd) throws OrmException {
  ...
  private Object indexGroup(Command cmd) throws OrmException {
    IndexGroup indexGroup = (IndexGroup) cmd;
    try {
      forwardedIndexGroupHandler.index(
          AccountGroup.UUID.parse(indexGroup.getId()), Operation.INDEX);
  ...
  private boolean evictCache(Command cmd) throws CacheNotFoundException {
  ...
  private Object forwardEvent(Command cmd) {
