...
abstract class ReindexRunnable<T> implements Runnable {
  ...
  private final IndexTs.IndexName itemName;
  private final OneOffRequestContext ctx;
  private final IndexTs indexTs;

  @Inject
  public ReindexRunnable(IndexTs.IndexName itemName, IndexTs indexTs, OneOffRequestContext ctx) {
    ...
    if (maybeIndexTs.isPresent()) {
      ...
      try (ManualRequestContext mctx = ctx.open()) {
        try (ReviewDb db = mctx.getReviewDbProvider().get()) {
          ...
          if (count > 0) {
            log.info("{} {}s reindexed", count, itemNameString);
          }
