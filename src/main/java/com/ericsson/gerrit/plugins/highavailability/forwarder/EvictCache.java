...
/**
 * Evict cache entries. This class is meant to be used on the receiving side of
 * the {@link Forwarder} since it will prevent cache evictions to be
 * forwarded again causing an infinite forwarding loop between the 2 nodes.
 */
@Singleton
public class EvictCache {
  ...
  /**
   * Evict an entry from the cache of the local node, eviction will not be
   * forwarded to the other node.
   *
   * @param pluginName the plugin name to which belong the cache or "gerrit" for
   * a Gerrit core cache
