...
public class ChangeReindexRunnable extends ReindexRunnable<Change> {
  ...
  @Inject
  public ChangeReindexRunnable(ChangeIndexer changeIdx, IndexTs indexTs, OneOffRequestContext ctx) {
    super(IndexName.CHANGE, indexTs, ctx);
  ...
  protected Optional<Timestamp> indexIfNeeded(ReviewDb db, Change c, Timestamp sinceTs) {
    try {
      ...
      if (changeTs.after(sinceTs)) {
        log.info("Index {}/{}/{}", c.getProject(), c.getId(), changeTs);
