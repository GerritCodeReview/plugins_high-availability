...
public class ForwardedIndexChangeHandler extends ForwardedIndexingHandler<String> {
  ...
  private void doIndex(String id, Optional<?> maybeBody, int retryCount)
    ...
    try (ReviewDb db = schemaFactory.open()) {
      ...
      if (change != null) {
        long changeTs = computeLastChangeTs(db, change);
        ...
        if (isChangeUpToDate(changeTs, indexEvent, changeTsGraceInterval)) {
          ...
          log.warn(
              ...
              id,
              indexEvent,
              ChangeIndexedEvent.format(changeTs));
  ...
  private boolean isChangeUpToDate(
      long changeTs, Optional<ChangeIndexedEvent> indexEvent, int changeTsGraceInterval) {
    return indexEvent.map(e -> changeTs >= (e.eventCreatedOn - changeTsGraceInterval)).orElse(true);
  }
