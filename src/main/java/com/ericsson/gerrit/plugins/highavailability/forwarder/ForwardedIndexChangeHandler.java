...
public class ForwardedIndexChangeHandler extends ForwardedIndexingHandler<String> {
  ...
  protected void doIndex(String id, Optional<Object> maybeBody) throws IOException, OrmException {
    ...
    try (ReviewDb db = schemaFactory.open()) {
      ...
      if (change != null) {
        ...
        if (isChangeUpToDate(change, indexEvent)) {
          log.debug("Change {} successfully indexed", id);
        } else {
          log.warn(
              "Change {} is too old compared compared to the eventys (eventTs={} >> changeTs={})",
              id,
              indexEvent,
  ...
  private boolean isChangeUpToDate(ChangeNotes change, Optional<ChangeIndexedEvent> indexEvent) {
    final long changeTs = computeLastChangeDataTimestamp(change);

    return indexEvent
        .map(e -> new Boolean((changeTs / 1000) >= (e.eventCreatedOn - CHANGE_TS_GRACE_PERIOD)))
        .orElse(true);
  }

  private long computeLastChangeDataTimestamp(ChangeNotes change) {
    long changeTs = change.getChange().getLastUpdatedOn().getTime();
    Multimap<RevId, Comment> comments = change.getComments();
    if (comments == null) {
      return changeTs;
    }

    for (Comment comment : comments.values()) {
      long commentTs = comment.writtenOn.getTime();
      changeTs = Math.max(changeTs, commentTs);
    }
    return changeTs;
  }
