...
public class ChangeCheckerImpl implements ChangeChecker {
  ...
  private boolean isEventShaInChangeMetaHistory(Repository repo, IndexEvent indexEvent)
      throws IOException {
    ObjectId metaObj = getMetaObj(repo);
    if (metaObj.name().equals(indexEvent.metaSha)) {
      return true;
    }

    RevCommit metaCommit =
        RevCommit.parse(repo.open(metaObj, Constants.OBJ_COMMIT).getCachedBytes());
    if (metaCommit.getParentCount() != 1) {
      return false;
    }

    do {
      metaObj = metaCommit.getParents()[0].toObjectId();
      metaCommit = RevCommit.parse(repo.open(metaObj, Constants.OBJ_COMMIT).getCachedBytes());
    } while (metaCommit.getParentCount() == 1
        && !metaObj.name().equals(indexEvent.metaSha)
