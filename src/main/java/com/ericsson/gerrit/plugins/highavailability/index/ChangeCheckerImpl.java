...
public class ChangeCheckerImpl implements ChangeChecker {
  ...
  @Override
  public Optional<ChangeIndexedEvent> newChangeIndexEvent() throws IOException, OrmException {
    return getComputedChangeTs()
        .map(
            ts -> {
              ChangeIndexedEvent event = new ChangeIndexedEvent();
  ...
  private String getBranchTargetSha() {
    try {
      try (Repository repo = gitRepoMgr.openRepository(changeNotes.get().getProjectName())) {
        return repo.findRef(changeNotes.get().getChange().getDest().get())
            .getTarget()
            .getObjectId()
            .getName();
      }
