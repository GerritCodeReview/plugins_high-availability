...
class ReplicationRestApiServlet extends AbstractRestApiServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse rsp) {
    ...
    try {
      ...
      ProjectEvent received = getEventFromRequest(req);
      log.info(
          "OFFLOADING --- received event: "
              + received.getProjectName()
              + " --- "
              + received.getClass().getCanonicalName());
      forwardedReplicationHandler.replicate(received);
      rsp.setStatus(SC_NO_CONTENT);
    } catch (IOException e) {
      log.error("Unable to start replication", e);
  ...
  private static ProjectEvent getEventFromRequest(HttpServletRequest req) throws IOException {
    ...
    Gson gson =
        new GsonBuilder()
            .registerTypeAdapter(ProjectEvent.class, new Deserializer())
