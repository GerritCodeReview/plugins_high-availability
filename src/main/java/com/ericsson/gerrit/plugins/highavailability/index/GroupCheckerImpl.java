...
class GroupCheckerImpl implements GroupChecker {
  ...
  public boolean isGroupUpToDate(
      AccountGroup.UUID groupUUID, Optional<IndexEvent> groupIndexEvent) {
    if (!groupIndexEvent.isPresent()) {
      logger.atWarning().log("Group Index empty, considering this group up-to-date");
      return true;
    }
    IndexEvent event = groupIndexEvent.get();

    if (event.targetSha == null) {
    ...
    try (Repository repo = repoManager.openRepository(allUsers)) {
      if (commitExistsInRepo(repo, ObjectId.fromString(event.targetSha))) {
        logger.atInfo().log(
            "Group '%s' up-to-date: sha1 '%s' exists in All-Users", groupUUID, event.targetSha);
        return true;
      } else {
        logger.atWarning().log(
            "Group '%s' NOT up-to-date: sha1 '%s' still missing in All-Users",
            groupUUID, event.targetSha);
      }
    } catch (Exception e) {
  ...
  @Override
  public ObjectId getGroupHead(AccountGroup.UUID groupUUID) {
    try (Repository repo = repoManager.openRepository(allUsers)) {
      return Optional.ofNullable(
              repo.exactRef(RefNames.refsGroups(AccountGroup.uuid(groupUUID.get()))))
          .map(Ref::getObjectId)
          .orElse(ObjectId.zeroId());
    } catch (Exception e) {
      logger.atSevere().withCause(e).log("Fatal: could not get head of group %s.", groupUUID.get());
