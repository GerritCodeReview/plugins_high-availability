...
public abstract class Results {
  ...
  public static Results create(List<Result> results) {
    Map<RequestStatus, List<Result>> requestsPerStatus =
        results.stream().collect(Collectors.groupingBy(result -> result.status()));
    return new AutoValue_Results(
        getRequestsByStatus(requestsPerStatus, RequestStatus.SUCCESSFULL),
        getRequestsByStatus(requestsPerStatus, RequestStatus.FAILURE),
        getRequestsByStatus(requestsPerStatus, RequestStatus.RETRY));
  }
