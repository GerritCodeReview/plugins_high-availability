 ...
 * creates its own channel and joins jgroup cluster.
 * Whenever the set of cluster members changes each gerrit server publishes
 * its url to all cluster members (publishes it to all channels).
 *
  ...
  public void viewAccepted(View view) {
    ...
    synchronized (this) {
      if (view.getMembers().size() > 2) {
        log.warn(
            "{} members joined the jgroups cluster {}. Only two members are supported. Members: {}",
    ...
    if (view.size() > 1) {
      try {
        ...
        log.error(
            "Sending a message over channel {} to cluster {} failed", channel.getName(), jgroupsConfig.clusterName(), e);
      }
    }
  }
