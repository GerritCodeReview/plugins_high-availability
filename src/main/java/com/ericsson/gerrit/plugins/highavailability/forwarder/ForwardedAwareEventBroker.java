...
class ForwardedAwareEventBroker extends EventBroker {
  ...
  private final AllowedUnrestrictedEventListeners allowedListeners;
  ...
  ForwardedAwareEventBroker(
      ...
      AllowedUnrestrictedEventListeners allowedListeners) {
    super(
        listeners,
  ...
  private void fireEventForListener(EventListener l, Event event) {
    if (!Context.isForwardedEvent() || allowedListeners.isAllowed(l)) {
      l.onEvent(event);
    }
  }
}
