...
public class NewProjectCreatedTrigger
    extends Trigger<NewProjectCreatedListener, ForwardedNewProjectCreatedEvent> {
  @Inject
  NewProjectCreatedTrigger(DynamicSet<NewProjectCreatedListener> listeners) {
    super(listeners);
  }

  public void callback(ForwardedNewProjectCreatedEvent event) {
    getListener().onNewProjectCreated(event);
  }
}
