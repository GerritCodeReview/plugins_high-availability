...
public class GcpPubSubForwarderModule extends LifecycleModule {
  ...
  @Singleton
  @GCloudProject
  public String getGCloudProject(Configuration config) {
    return config.pubSubGcp().gCloudProject();
  }
  ...
  public static ExecutorProvider buildPublisherExecutorProvider(Configuration config) {
    return InstantiatingExecutorProvider.newBuilder()
        .setExecutorThreadCount(config.pubSubGcp().publisherThreadPoolSize())
        .build();
  }
  ...
  public static ExecutorProvider buildSubscriberExecutorProvider(Configuration config) {
    return InstantiatingExecutorProvider.newBuilder()
        .setExecutorThreadCount(config.pubSubGcp().subscriberThreadPoolSize())
        .build();
  }
  ...
  @Singleton
  @DefaultTopic
  Publisher createDefaultTopicPublisher(
      PublisherFactory factory, @GCloudProject String gCloudProject, TopicNames topicNames) {
    return factory.create(TopicName.of(gCloudProject, topicNames.defaultTopic()));
  }

  @Provides
  @Singleton
  @StreamEventsTopic
  Publisher createStreamEventsTopicPublisher(
      PublisherFactory factory, @GCloudProject String gCloudProject, TopicNames topicNames) {
    return factory.create(TopicName.of(gCloudProject, topicNames.streamEventsTopic()));
  }
