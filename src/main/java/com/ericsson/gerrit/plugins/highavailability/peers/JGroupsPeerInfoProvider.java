...
public class JGroupsPeerInfoProvider extends ReceiverAdapter
    ...
    synchronized (this) {
      if (view.getMembers().size() > 2) {
        log.warn(
            ...
            view.getMembers().size(), channel.getName(), view.getMembers());
      }
   ...
   * whose name matches {@link #skipInterfacePattern} are also ignored. By that it is possible to
   * skip interfaces which should not be used by jgroups (e.g. 'lo0', 'utun0' on MacOS).
   *
  ...
  private Optional<InetAddress> findDefaultAddress() throws SocketException {
    ...
    while (networkInterfaces.hasMoreElements()) {
      ...
      while (inetAddresses.hasMoreElements()) {
        ...
        if (!preferIPv4 && ret instanceof Inet6Address) {
          return Optional.of(ret);
        }
      }
    }
