...
public class ForwardedCacheEvictionHandlerIT extends LightweightPluginDaemonTest {
  ...
  @Test
  public void shouldNotEvictProjectCache() throws Exception {
    String newProject = name("newProject");
    gApi.projects().create(newProject);

    Object parsedKey = gsonParser.fromJson(ProjectCacheImpl.CACHE_NAME, gson.toJson(newProject));
    objectUnderTest.evict(CacheEntry.from(ProjectCacheImpl.CACHE_NAME, parsedKey));
    evictionsCacheTracker.waitForExpectedEvictions();
