...
public class RestForwarderTest {
  ...
  // Replication
  private static ProjectEvent replicationEvent = new ForwardedProjectDeletedEvent("myproject");
  private static final String REPLICATION_ENDPOINT =
      Joiner.on("/")
          .join(
              ...
              PLUGIN_NAME,
              "replication",
              "com.ericsson.gerrit.plugins.highavailability.replication.events.ForwardedProjectDeletedEvent");
  private static String replicationJson =
      "{\"class\":\"com.ericsson.gerrit.plugins.highavailability.replication.events.ForwardedProjectDeletedEvent\"}";
  ...
  public void testReplicationSentOK() throws Exception {
    when(httpSessionMock.post(eq(REPLICATION_ENDPOINT), anyString()))
        .thenReturn(new HttpResult(SUCCESSFUL, EMPTY_MSG));
    assertThat(
  ...
  public void testReplicationSentThrowsException() throws Exception {
    doThrow(new IOException()).when(httpSessionMock).post(eq(REPLICATION_ENDPOINT), anyString());
    assertThat(
            forwarder.replicate(replicationEvent, replicationEvent.getClass().getCanonicalName()))
        .isFalse();
  }
