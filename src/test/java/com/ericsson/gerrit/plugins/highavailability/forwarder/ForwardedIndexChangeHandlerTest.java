...
public class ForwardedIndexChangeHandlerTest {
  ...
  @Test
  public void changeIsNotIndexedWhenOutdated() throws Exception {
    setupChangeAccessRelatedMocks(CHANGE_EXISTS, CHANGE_OUTDATED);
    handler.index(TEST_CHANGE_ID, Operation.INDEX);
    verify(indexerMock, never()).index(any(ReviewDb.class), any(Change.class));
  }
  ...
  private void setupChangeAccessRelatedMocks(
    ...
    when(changeCheckerMock.isChangeUpToDate(any(), anyInt())).thenReturn(changeIsUpToDate);
  }
}
